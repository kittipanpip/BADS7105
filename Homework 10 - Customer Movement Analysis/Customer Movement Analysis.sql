WITH cust_month as (
    ...
    FROM `airy-generator-315513.Supermarket_Data.supermarket_data`
    where CUST_CODE is not null 
)
SELECT report_month,
    count(distinct case when status = 'New' THEN CUST_CODE ELSE NULL END) as New_cust,
    count(distinct case when status = 'Repeat' THEN CUST_CODE ELSE NULL END) as Frequenter,
    count(distinct case when status = 'Reactivated' THEN CUST_CODE ELSE NULL END) as Reactivated_cust,
    count(distinct case when status = 'Churn' THEN CUST_CODE ELSE NULL END) as Churn_cust
FROM (
    SELECT CUST_CODE, 
